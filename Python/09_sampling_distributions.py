"""
Sampling and Sampling Distributions - Python Code Examples
Corresponds to: 09_sampling_distributions.md
"""
...
def simple_random_sampling():
    """
    Demonstrate simple random sampling with mathematical precision.
    """
    ...
    return data, random_sample, sample_mean, population_mean

def systematic_sampling():
    """
    Demonstrate systematic sampling with mathematical explanation.
    """
    # Load data
    iris = load_iris()
    data = pd.DataFrame(iris.data, columns=iris.feature_names)
    ...
    return systematic_sample, sys_mean

def stratified_sampling():
    """
    Demonstrate stratified sampling by target class with mathematical precision.
    """
    # Load data
    iris = load_iris()
    data = pd.DataFrame(iris.data, columns=iris.feature_names)
    ...
    print(f"Simple Random Sample mean: {simple_random_sampling()[2]:.4f}")
    print(f"Systematic Sample mean: {systematic_sampling()[1]:.4f}")
    print(f"Stratified Sample mean: {strat_mean:.4f}")
    print(f"Population mean: {population_mean:.4f}")
    
    return stratified_sample, strat_mean
...
def simulate_sampling_distribution():
    ...
    return sample_means, population_mean, theoretical_se

def visualize_sampling_distribution():
    """
    Comprehensive visualization of sampling distribution.
    """
    # Get sampling distribution data
    sample_means, population_mean, theoretical_se = simulate_sampling_distribution()
...
def bootstrap_different_statistics():
    """
    Bootstrap for different statistics with mathematical interpretation.
    """
    # Load data
    iris = load_iris()
    data = pd.DataFrame(iris.data, columns=iris.feature_names)
    ...
    # Bootstrap for mean (recreate for this function)
    def boot_mean(data, indices):
        return np.mean(data[indices])
...
def compare_sampling_methods():
    ...
    def compare_sampling_methods_internal(data, variable, sample_size, n_simulations=100):
    ...
    sampling_comparison = compare_sampling_methods_internal(data, "sepal length (cm)", 10)
...
def quality_control_example():
    """
    Quality control example with mathematical precision.
    """
...
def survey_sampling_example():
    """
    Survey sampling example with statistical precision.
    """
...
def check_sampling_bias():
    ...
    def check_sampling_bias_internal(population, sample_data, variable):
    ...
    bias_check = check_sampling_bias_internal(population_data, sample_data, "sepal length (cm)")
...
if __name__ == "__main__":
    ...
    variance_sampling()
    bootstrap_sampling()
    bootstrap_different_statistics()
    compare_sampling_methods()
    quality_control_example()
    survey_sampling_example()
